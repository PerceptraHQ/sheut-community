...
use sheut_core::{
    AnalyticConfidence, BrandProfile, BuiltinReportTemplate, DocumentActivityKind,
    DocumentEnvelope, DocumentKind, EvidenceFileMetadata, EvidenceMetadataInput, GraphViewport,
    GraphWorkspace, GuidedReport, GuidedReportFieldValue, ImageAttachmentMetadata, ImageMediaType,
    LocalId, MitreCatalog, MitreTechniqueReference, Position, ProjectMetadata, PublicationFormat,
    PublicationRecord, PublicationSettings, PublicationSnapshot, PublicationSource, Revision,
    ...
    built_in_report_templates, report_template_revision,
};
use sheut_stix::{
...
#[test]
fn migration_ten_preserves_documents_and_round_trips_revisioned_publication_data() {
    let directory = TestDirectory::new();
    let path = directory.path("project.sheut");
    let original_document = fixture_document(1);
    ...
    let snapshot = PublicationSnapshot::new(
        ...
        PublicationSource::GuidedReport {
            report_id: revised_report.id(),
            revision: revised_report.revision(),
        },
        settings,
        3_000,
    )
    ...
    {
        let mut store = EncryptedStore::create(&path, &PROJECT_KEY).unwrap();
        store.save_document(&original_document, None).unwrap();
        store.save_report_template(&template, None).unwrap();
        store.save_guided_report(&report, None).unwrap();
        store
            .save_guided_report(&revised_report, Some(Revision::new(1).unwrap()))
            .unwrap();
        store.save_brand_profile(&brand, None).unwrap();
        store.save_publication_record(&record).unwrap();
    }
    ...
    assert_eq!(
        store
            .load_document(LocalId::parse(DOCUMENT_ID).unwrap())
            .unwrap(),
        Some(original_document.clone())
    );
    assert_eq!(
        store
            .load_guided_report(LocalId::parse(SECOND_DOCUMENT_ID).unwrap())
            .unwrap(),
        Some(revised_report.clone())
    );
    assert_eq!(
        store.list_report_templates().unwrap(),
        vec![template.clone()]
    );
    assert_eq!(
        store
            .load_report_template_revision(template.id(), template.revision())
            .unwrap(),
        Some(template.clone())
    );
    assert_eq!(
        store
            .load_guided_report_revision(report.id(), report.revision())
            .unwrap(),
        Some(report)
    );
    assert_eq!(
        store
            .load_guided_report_revision(revised_report.id(), revised_report.revision())
            .unwrap(),
        Some(revised_report.clone())
    );
    assert_eq!(
        store
            .load_document_revision_for_publication(
    ...
    assert_eq!(store.list_brand_profiles().unwrap(), vec![brand]);
    assert_eq!(store.list_publication_records().unwrap(), vec![record]);
}

#[test]
fn illicit_ecosystem_upgrade_persists_as_a_new_recoverable_revision() {
    let directory = TestDirectory::new();
    ...
    let original = blank
        ...
        GuidedReportFieldValue::Narrative(serde_json::json!({
            ...
            "content": [{
                "type": "paragraph",
                "content": [{"type": "text", "text": "The source-reported claim remains uncorroborated."}]
            }]
        })),
        2_000,
    )
    ...
    {
        let mut store = EncryptedStore::create(&path, &PROJECT_KEY).unwrap();
        store.save_guided_report(&blank, None).unwrap();
        store
            .save_guided_report(&original, Some(blank.revision()))
            .unwrap();
        store
            .save_guided_report(&upgraded, Some(original.revision()))
            .unwrap();
    }

    let store = EncryptedStore::open(&path, &PROJECT_KEY).unwrap();
    assert_eq!(
        store.load_guided_report(upgraded.id()).unwrap(),
        Some(upgraded)
    );
    assert_eq!(
        store
            .load_guided_report_revision(original.id(), original.revision())
            .unwrap(),
        Some(original)
    );
}
...
#[test]
fn guided_report_revisions_reject_stale_writers() {
    let directory = TestDirectory::new();
    let path = directory.path("project.sheut");
    let mut store = EncryptedStore::create(&path, &PROJECT_KEY).unwrap();
    let template = built_in_report_templates()
    ...
    let error = store
        .save_guided_report(&report, Some(Revision::new(1).unwrap()))
        .unwrap_err();
...
#[test]
fn guided_report_soft_deletion_is_recoverable() {
    let directory = TestDirectory::new();
    let path = directory.path("project.sheut");
    let mut store = EncryptedStore::create(&path, &PROJECT_KEY).unwrap();
    let template = built_in_report_templates()
    ...
    assert!(store.soft_delete_guided_report(report.id(), 2_000).unwrap());
    assert!(store.list_guided_reports().unwrap().is_empty());
    assert_eq!(
        store.list_all_guided_reports().unwrap(),
        vec![report.clone()]
    );
    assert_eq!(store.load_guided_report(report.id()).unwrap(), None);
    assert!(!store.soft_delete_guided_report(report.id(), 2_100).unwrap());

    assert!(store.restore_guided_report(report.id()).unwrap());
    assert_eq!(
        store.load_guided_report(report.id()).unwrap(),
        Some(report.clone())
    );
    assert!(!store.restore_guided_report(report.id()).unwrap());
}
