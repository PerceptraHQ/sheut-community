...
fn render_html_node(node: &Value, output: &mut String) {
    ...
    match node_type {
        ...
        "graphSnapshot" => render_html_graph_snapshot(object, output),
        "evidenceCitation" => render_html_evidence_citation(object, output),
        "projectReference" => render_html_project_reference(object, output),
        "mitreSnapshot" => render_html_mitre_snapshot(object, output),
...
fn render_html_project_reference(object: &serde_json::Map<String, Value>, output: &mut String) {
    let Some(attrs) = object.get("attrs").and_then(Value::as_object) else {
        return;
    };
    let Some(source_id) = attrs.get("sourceId").and_then(Value::as_str) else {
        return;
    };
    let Some(label) = attrs.get("label").and_then(Value::as_str) else {
        return;
    };
    output.push_str("<span data-sheut-project-reference=\"");
    output.push_str(&escape_html(source_id));
    output.push_str("\">");
    output.push_str(&escape_html(label));
    output.push_str("</span>");
}

fn render_html_graph_snapshot(object: &serde_json::Map<String, Value>, output: &mut String) {
    let Some(attrs) = object.get("attrs").and_then(Value::as_object) else {
        return;
    };
    ...
    output.push_str("<figure data-sheut-graph-snapshot=\"");
...
fn render_html_mitre_snapshot(object: &serde_json::Map<String, Value>, output: &mut String) {
    ...
    output.push_str("<table data-sheut-mitre-snapshot><thead><tr><th>Technique ID</th><th>Technique</th><th>Explanation</th></tr></thead><tbody>");
    for observation in observations {
        output.push_str("<tr>");
        for field in ["techniqueId", "techniqueName", "explanation"] {
            output.push_str("<td>");
...
fn render_html_evidence_citation(object: &serde_json::Map<String, Value>, output: &mut String) {
    let Some(attrs) = object.get("attrs").and_then(Value::as_object) else {
        return;
    };
    let Some(evidence_id) = attrs.get("evidenceId").and_then(Value::as_str) else {
        return;
    };
    let Some(label) = attrs.get("label").and_then(Value::as_str) else {
        return;
    };
    output.push_str("<span data-sheut-evidence-citation=\"");
    output.push_str(&escape_html(evidence_id));
    output.push_str("\">[");
    output.push_str(&escape_html(label));
    output.push_str("]</span>");
}
