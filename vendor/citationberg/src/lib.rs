...
use taxonomy::{
    DateVariable, Kind, Locator, NameVariable, NumberOrPageVariable, OtherTerm, Term, Variable,
};
...
impl IndependentStyle {
    ...
    pub fn from_xml(xml: &str) -> XmlDeResult<Self> {
        ...
        IndependentStyle::deserialize(de).map_err(|source| XmlDeError {
            source,
...
impl DependentStyle {
    ...
    pub fn from_xml(xml: &str) -> XmlDeResult<Self> {
        ...
        let mut track = serde_path_to_error::Track::new();
        let de = serde_path_to_error::Deserializer::new(&mut de, &mut track);
        DependentStyle::deserialize(de).map_err(|source| XmlDeError {
            source,
            path: Some(track.path()),
        })
    }
...
impl Style {
    ...
    pub fn from_xml(xml: &str) -> XmlDeResult<Self> {
        ...
        let mut track = serde_path_to_error::Track::new();
        let de = serde_path_to_error::Deserializer::new(&mut de, &mut track);
        Style::deserialize(de).map_err(|source| XmlDeError {
            source,
            path: Some(track.path()),
        })
    }
    ...
    pub fn to_xml(&self) -> XmlSeResult<String> {
        ...
        self.serialize(ser).map_err(|source| XmlSeError {
            source,
            path: Some(track.path()),
        })?;
        Ok(buf)
    ...
    pub fn format(
        ...
        if start_pre == end_pre {
            ...
            match self {
                ...
                PageRangeFormat::Chicago15 if x_len == 4 && changed_digits(x, &y) >= 3 => {
...
fn minimal(buf: &mut impl fmt::Write, thresh: usize, x: &str, y: &str) -> Result<(), fmt::Error> {
    if y.len() > x.len() {
...
impl Name {
    ...
    pub fn options<'s>(&'s self, inherited: &'s InheritableNameOptions) -> NameOptions<'s> {
...
impl ChooseBranch {
    ...
    pub fn test(&self) -> Option<ChooseTest<'_>> {
        if let Some(disambiguate) = self.disambiguate {
            ...
            self.variable
                .as_ref()
                .map(|variable| ChooseTest::Variable(variable))
        }
    }
}
...
impl LocaleFile {
    ...
    pub fn from_xml(xml: &str) -> XmlDeResult<Self> {
        ...
        let mut track = serde_path_to_error::Track::new();
        let de = serde_path_to_error::Deserializer::new(&mut de, &mut track);
        let locale: Self = Self::deserialize(de).map_err(|err| XmlDeError {
            source: err,
            path: Some(track.path()),
        })?;
        Ok(locale)
    ...
    pub fn to_xml(&self) -> XmlSeResult<String> {
        let mut buf = String::new();
        let ser = quick_xml::se::Serializer::with_root(&mut buf, Some("style")).map_err(|err| {
            XmlSeError {
                source: err,
                path: None,
            }
        ...
        let mut track = serde_path_to_error::Track::new();
        let ser = serde_path_to_error::Serializer::new(ser, &mut track);
        self.serialize(ser).map_err(|err| XmlSeError {
            source: err,
            path: Some(track.path()),
        })?;
        Ok(buf)
